...
static const rclcpp::Logger LOGGER = rclcpp::get_logger("demo_mycobot_320pi");
...
mtc::Task MTCTaskNode::createTask()
{
  ...
  const auto &hand_group_name = "gripper_group";
...
#pragma GCC diagnostic pop

  {
...

    {
      ...
      stage->setMinMaxDistance(0.09, 0.1);
    ...
    }
    {
      ...
      stage->properties().configureInitFrom(mtc::Stage::PARENT, {"group"});
      stage->setMinMaxDistance(0.09, 0.15);
  ...
  // Serial container for the place stages
  {
...

    {
      ...
      wrapper->setMaxIKSolutions(2);
      wrapper->setMinSolutionDistance(0.005);
      wrapper->setIKFrame("object");
      wrapper->properties().configureInitFrom(mtc::Stage::PARENT, {"eef", "group"});
      wrapper->properties().configureInitFrom(mtc::Stage::INTERFACE, {"target_pose"});
